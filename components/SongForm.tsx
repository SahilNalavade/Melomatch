...
const SongForm: React.FC<SongFormProps> = ({ onSubmit }) => {
  ...
  const debouncedSearch = _.debounce(async (input: string) => {
    try {
      ...
      const response = await axios.get(`https://ws.audioscrobbler.com/2.0/?method=track.search&track=${input}&api_key=91e0b0d00eefb1d4b39d5a6b44f1ba75&format=json`);
      ...
      if (response.data.results && response.data.results.trackmatches) {
        ...
        const songs = response.data.results.trackmatches.track.map((item: any) => item.name);
  ...
  return (
    <div>
      <input type="text" placeholder="Enter username" value={usernameInput} onChange={handleUsernameChange} />
      <input type="text" placeholder="Enter song" value={songInput} onChange={handleInputChange} />
      
      <h2>Search Results</h2>
      <ul>
        {searchResults.map((result, index) => (
          <li key={index} onClick={() => handleSelectSong(result)}>
            {result}
          </li>
        ))}
      </ul>
